...
# ================== ЛОГИКА ПРОГРЕССА (УДАЛЕНИЕ + ОТПРАВКА) ==================
...
async def delete_old_progress(message: Message, state: FSMContext):
    """Удаляет предыдущее сообщение с прогрессом, если оно есть."""
    data = await state.get_data()
    progress_msg_id = data.get("progress_msg_id")
    if progress_msg_id:
        try:
            await message.bot.delete_message(chat_id=message.chat.id, message_id=progress_msg_id)
            logger.info(f"🗑️ Удалено старое сообщение прогресса (ID {progress_msg_id})")
        except Exception as e:
            logger.warning(f"Не удалось удалить прогресс (ID {progress_msg_id}): {e}")
        await state.update_data(progress_msg_id=None)

async def send_new_progress(message: Message, user_id: int, state: FSMContext, short_type: str, short_level: str):
    """Удаляет старый прогресс и отправляет новый."""
    await delete_old_progress(message, state)
    ...
    logger.info(f"✅ Отправлено новое сообщение прогресса (ID {sent_msg.message_id})")
...
async def choose_level(callback: CallbackQuery, state: FSMContext):
    ...
    # Отправляем прогресс (удаляем старое, если есть, и отправляем новое)
    await send_new_progress(callback.message, user_id, state, short_type, short_level)
    # Отправляем задание
...
async def handle_button_answer(callback: CallbackQuery, state: FSMContext):
    ...
    # Обновляем прогресс (удаляем старый, отправляем новый)
    await send_new_progress(callback.message, user_id, state, short_type, short_level)
...
async def handle_text_answer(message: Message, state: FSMContext):
    ...
    # Обновляем прогресс
    await send_new_progress(message, user_id, state, short_type, short_level)
...
async def confirm_reset(callback: CallbackQuery, state: FSMContext):
    ...
    # Отправляем обновлённый прогресс (0/0)
    await send_new_progress(callback.message, user_id, state, short_type, short_level)
