...
# Импорт данных из нового файла (словарь с категориями)
from data.grammar_tasks import GRAMMAR_TASKS

# Импорт функций работы с БД для прогресса грамматики
...
class GrammarStates(StatesGroup):
    active = State()   # пользователь в режиме грамматики

# ---------- Загрузка заданий ----------
def load_all_tasks():
    """
    Склеивает все задания из всех категорий в один плоский список.
    Предполагается, что GRAMMAR_TASKS = {
        'category1': [ {...}, {...} ],
        'category2': [ {...}, ... ],
        ...
    }
    """
    all_tasks = []
    for category, tasks in GRAMMAR_TASKS.items():
...
async def show_grammar_task(message: Message, user_id: int, state: FSMContext, edit: bool = False):
    """Показывает текущее задание (берёт индекс из БД)."""
...
# ---------- Вход в режим (вызывается из главного меню) ----------
async def enter_grammar_mode(message: Message, user_id: int, state: FSMContext, edit: bool = False):
    """Точка входа в режим грамматики."""
    await state.set_state(GrammarStates.active)
    await state.update_data(mode="grammar")   # дополнительная метка (опционально)
    await show_grammar_task(message, user_id, state, edit)

# ---------- Хендлер кнопки "Показать ответ" ----------
...
async def finish_grammar(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.answer()
    from handlers.start import show_main_menu   # или из вашего модуля
...
async def handle_grammar_answer(message: Message, state: FSMContext):
    ...
    if isinstance(correct, str):
        ...
        display_answer = correct[0]   # первый вариант для показа
...
# ---------- Защита от сообщений вне режима ----------
