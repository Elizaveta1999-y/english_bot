...
def update_streak(user_id: int):
    """Обновляет дни подряд (streak). Создаёт профиль, если его нет."""
    state = get_user_state(user_id)
    ...
    if "profile" not in state:
        state["profile"] = {
            ...
            "subscription": {"active": False, "expires": None},
            "settings": {"notifications": True, "notification_time": "10:00"}
        }
...
def get_settings_keyboard():
    settings = get_user_state(callback.from_user.id).get("profile", {}).get("settings", {})
...
async def profile_menu(callback: CallbackQuery):
    ...
    # Сначала создаём профиль, затем обновляем streak
    stats = get_stats(user_id)
    update_streak(user_id)  # теперь безопасно
