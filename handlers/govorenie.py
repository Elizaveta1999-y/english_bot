...
# ---------- Загрузка заданий ----------
TASKS_FILE = os.path.join(os.path.dirname(__file__), "../data/speaking_tasks.json")
...
async def get_speaking_index(user_id: int, task_type: str, level: str) -> int:
    r = await get_redis()
    key = f"govorenie_progress:{user_id}:{task_type}:{level}"
    val = await r.get(key)
    return int(val) if val else 0

async def set_speaking_index(user_id: int, task_type: str, level: str, index: int):
    r = await get_redis()
    key = f"govorenie_progress:{user_id}:{task_type}:{level}"
...
async def show_task(message: Message, state: FSMContext, edit: bool = False):
    ...
    if task_type == "reading":
        task_text = (
            f"Задание {index+1} из {total} (Чтение вслух)\n\n"
            f"Прочитайте следующий текст чётко и с интонацией:\n\n"
            f"_{task['text']}_"
        )
    elif task_type == "fluency":
        task_text = (
            f"Задание {index+1} из {total} (Беглость)\n\n"
            f"Говорите без остановок не менее 60 секунд на тему:\n\n"
            f"**{task['topic']}**"
        )
    elif task_type == "interview":
        questions = "\n".join([f"{i+1}. {q}" for i, q in enumerate(task['questions'])])
        task_text = (
            f"Задание {index+1} из {total} (Интервью)\n\n"
            f"Ответьте на все вопросы одним голосовым сообщением:\n\n"
            f"{questions}"
        )
    else:
...
async def go_to_next_task(message: Message, state: FSMContext, user_id: int, task_type: str, level: str, current_index: int, tasks: list):
    ...
    await set_speaking_index(user_id, task_type, level, next_index)
    await state.update_data(
        index=next_index,
