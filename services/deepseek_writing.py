...
async def check_writing(task_text: str, user_answer: str, level: str, keywords: list, task_type: str) -> tuple:
    ...
    prompt = (
        ...
        "ВАЖНО: все советы должны быть в ОДНОМ теге <blockquote>...</blockquote>. "
        "Каждый совет начинай с НОВОЙ СТРОКИ (между советами ставь перенос строки). "
        "Каждый совет нумеруй: 1., 2., 3. "
        "НЕ разделяй советы на отдельные <blockquote> и НЕ оборачивай в цитату ничего другого (критерии, оценку, похвалу).\n"
        "Пример оформления советов (обрати внимание: каждый совет на своей строке):\n"
        "<b>Советы по улучшению:</b>\n"
        "<blockquote>1. Используй более разнообразную лексику.\n"
        "2. Обрати внимание на порядок слов.\n"
        "3. Добавь пример.</blockquote>\n"
        "Если текст действительно хорош (оценка 4 или 5), добавь ОДНУ короткую похвалу, и ВНУТРИ ЭТОГО ЖЕ ПРЕДЛОЖЕНИЯ поставь РОВНО ОДИН смайлик из набора: 👍🏻, 👏🏻, 🤩, 😉. "
        "Смайлик НЕ должен стоять отдельно или в конце ответа — только внутри предложения с похвалой. "
        "Например: 'Отличная работа, ты хорошо раскрыл тему 👍🏻'. "
        "Во всём ответе должен быть только один смайлик. Если текст не очень хорош — похвалу НЕ пиши, смайлик НЕ ставь.\n"
        ...
        "Не используй другие эмодзи, кроме разрешённого одного (и только внутри похвалы).\n"
    )
