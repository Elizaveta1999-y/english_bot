...
async def finish_roleplay(message: Message):
    ...
    if len(user_messages) < 3:
        ...
        await message.answer(f"📭 Вы ещё не общались. Отправьте ещё {needed} сообщения (нужно минимум 3).")
        return
    processing_msg = await message.answer("🔄 Генерирую анализ диалога...")
    conversation = "\n".join([f"{'User' if h['role']=='user' else 'Bot'}: {h['text']}" for h in history[-20:]])
    topic = user_state.get("roleplay_topic", "ролевая игра")
    await message.bot.send_chat_action(chat_id=message.chat.id, action="typing")
    prompt = (f"Ты преподаватель. Проанализируй диалог на тему '{topic}'. "
              "Дай короткий фидбек (5-7 предложений) на русском. Сначала похвали, потом ошибки с исправлениями, потом совет. Используй <b> и <i>. Добавь смайлики.\n\n"
              f"Диалог:\n{conversation}")
...
async def feedback_button(message: Message):
    ...
    history = user_state.get("history", [])
    if len(history) < 2:
        await message.answer("Вы ещё не общались.")
        return
    conversation = "\n".join([f"{'Student' if h['role']=='user' else 'Teacher'}: {h['text']}" for h in history[-10:]])
    prompt = ("Ты учитель английского. Дай короткий фидбек (до 5 предложений) на русском. Сначала похвали, потом ошибки с исправлениями, потом совет. Используй <b> и <i>. Добавь смайлики.\n\n"
              f"Диалог:\n{conversation}")
    await message.bot.send_chat_action(chat_id=message.chat.id, action="typing")
