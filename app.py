...
from aiogram.types import Update, Message, InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, ReplyKeyboardMarkup, KeyboardButton, ReplyKeyboardRemove
...
async def start_handler(message: Message):
    ...
    await message.answer("Привет! Это тестовый бот. Нажми кнопку.", reply_markup=keyboard)

@dp.callback_query(lambda c: c.data == "start_speaking")
async def test_callback(callback: CallbackQuery):
    await callback.message.answer("Вы нажали Speaking. Теперь попробуйте текстовые кнопки.")
    keyboard = ReplyKeyboardMarkup(keyboard=[
        [KeyboardButton(text="Кнопка 1"), KeyboardButton(text="Кнопка 2")]
    ], resize_keyboard=True)
    await callback.message.answer("Вот клавиатура:", reply_markup=keyboard)
...
async def test_roleplay(callback: CallbackQuery):
    await callback.message.answer("Ролевая игра в тестовом режиме.")
    await callback.answer()
