...
async def process_custom_scenario(message: Message):
    ...
    if len(scenario_text.split()) < 3:
        await message.answer(
            "❌ <b>Сценарий слишком короткий</b>. Опишите ситуацию подробнее (минимум 3 слова).\n\n"
            "Пример: <i>Ты продавец в книжном магазине. Я покупатель, ищу книгу по фантастике.</i>",
            parse_mode="HTML"
        )
        user_state["awaiting_custom_scenario"] = True
        set_user_state(user_id, user_state)
...
async def translate_caption(callback: CallbackQuery):
    ...
    if data.get("translation"):
        ...
        translation = chat(f"Translate the following English text to Russian. Output ONLY the translation, no extra words, no explanations, no quotes, no asterisks. Just the translation.\n\n{data['text']}", max_tokens=300, temperature=0.3)
