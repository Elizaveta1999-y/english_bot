...
async def get_short_feedback(conversation: str) -> str:
    """Генерирует сокращённый фидбек (ошибки, рекомендации, словарик) в читаемом формате."""
    prompt = f"""Ты опытный учитель американского английского. Проанализируй диалог и дай короткий фидбек на русском языке.

Диалог:
{conversation}

Формат ответа (используй эмодзи внутри текста):
<b>📝 Ошибки и исправления</b>
- <i>Ошибка:</i> -> <i>Исправление</i> (краткое правило)  (не более 3-4 пунктов)

<b>💡 Рекомендации по улучшению</b>
- 2 короткие фразы (например: "Больше слушай английскую речь", "Тренируй произношение звука th")

<b>📚 Словарик для изучения</b>
- слово / фраза — перевод (пример из 3-5 слов)

Не пиши лишнего. Будь кратким и полезным."""
