...
# ---------- Сброс статистики (добавлено) ----------
async def reset_user_stats(user_id: int, type_key: str, level_key: str):
    """Полностью удаляет статистику (правильные/неправильные) для данного типа и уровня."""
    r = await get_redis()
    key_correct = f"reading_stats:{user_id}:{type_key}:{level_key}:correct"
    key_wrong = f"reading_stats:{user_id}:{type_key}:{level_key}:wrong"
    await r.delete(key_correct, key_wrong)